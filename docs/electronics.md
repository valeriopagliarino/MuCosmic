...
Two different configurations for the optical gaseous detector readout have been designed:
- A CMOS camera connected via USB to the Red Pitaya board, managed by the Linux Operative System (with low resolution, considering the limited computing resources available on the Xilinx XC7Z010 SoC)
- An array of APD sensors in sum configuration, connected to the second RF input of the Red Pitaya board, or with a (rough) position sensitive configuration using a dedicated multi-channel readout board.
